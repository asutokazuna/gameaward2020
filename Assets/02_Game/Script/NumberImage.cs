...
public class NumberImage : MonoBehaviour
{
    ...
    /**
    * @brief        値のセット
    * @param[in]    int 表示したい数値 bool 表示したい方向　true：左 false：右
    * @return       なし
    * @details      表示したい値をセットする関数です
    */
    public void SetValue(int num, bool dir)
    {
        // 対象となるオブジェクトを探す
        ...
        {
            if (0 <= obj.name.IndexOf(_numberObject, 0))
            {
    ...
    */
    void RightView()
    {
        ...
        // 1桁目
        GameObject.Find(_numberObject).GetComponent<Image>().sprite = _numImage[_digitList[_digitList.Count - 1]];
        GameObject.Find(_numberObject).GetComponent<Image>().color = new Color(_numberColor.r, _numberColor.g, _numberColor.b, _numberColor.a);     // カラーのセット

        // 2桁目以降
        for (int i = _digitList.Count - 2; i > -1; i--)
        {
            RectTransform _numberImage = (RectTransform)Instantiate(GameObject.Find(_numberObject)).transform;                      // 初期ポジション
            _numberImage.SetParent(this.transform, false);                                                                          // 親の選択
            _numberImage.localPosition = new Vector2(                                                                               // ポジション指定
